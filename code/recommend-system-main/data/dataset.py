...
class UniSRecDataset(SequentialDataset):
    ...
    def load_plm_embedding(self):
        feat_path = os.path.join(self.config['data_path'], f'{self.plm_suffix}')
        loaded_feat = np.fromfile(feat_path, dtype=np.float32).reshape(-1, self.plm_size)

        data_maps_path = os.path.join(self.config['data_path'], f'data.maps')
...
class RichUniSRecDataset(UniSRecDataset):
    def __init__(self, config):
        super().__init__(config)
        self.struct_suffix = config['struct_suffix']
        self.struct_feature, self.struct_feature_dims = self.load_struct_feature()

    def load_struct_feature(self):
        struct_path = os.path.join(self.config['data_path'], self.struct_suffix)
        data_maps_path = os.path.join(self.config['data_path'], f'data.maps')
        with open(data_maps_path, 'r') as f:
            data_maps = json.load(f)

        dims = data_maps.get('struct_feature_dims', [1])
        mapped_struct = np.zeros((self.item_num, len(dims)), dtype=np.int64)
        if os.path.exists(struct_path):
            loaded_struct = np.load(struct_path)
            for i, token in enumerate(self.field2id_token['item_id']):
                if token == '[PAD]':
                    continue
                mapped_struct[i] = loaded_struct[int(data_maps['item2id'][token])]
        return torch.from_numpy(mapped_struct).long(), dims
